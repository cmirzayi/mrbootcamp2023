...
View(data.frame(dat$SNP,dat$effect_allele,dat$other_allele,dat$effect_allele.outcome,dat$other_allele.outcome,dat$swapt,
...
dmres<-metafor::rma.uni(yi=dat$Wald, vi=dat$Waldvar, slab=dat$lab, method="FE")
dmres
metafor::forest(dmres, atransf=exp,xlab=" ", mlab="Ischemic heart disease (OR)", at=log(c(.5, 1,2)),xlim=c(-1.7,1.3),cex=.8)
